...
class TestAddContextView:
    """Tests for the AddContextView.
    ...
    def test_is_importable(self):
        """Test that the AddContextView class has successfully been imported, but do not attmept to actually test the class"""
        # Assert the module was loaded
        assert "polychron.views.AddContextView" in sys.modules
        # Assert the specific class now exists, but do not instantiate it
        assert isinstance(AddContextView, type)
