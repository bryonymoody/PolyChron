...
class TestSplashView:
    """Tests for the SplashView.
    ...
    def test_is_importable(self):
        """Test that the SplashView class has successfully been imported, but do not attmept to actually test the class"""
        # Assert the module was loaded
        assert "polychron.views.SplashView" in sys.modules
        # Assert the specific class now exists, but do not instantiate it
        assert isinstance(SplashView, type)
