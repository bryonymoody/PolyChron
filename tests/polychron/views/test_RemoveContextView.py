...
class TestRemoveContextView:
    """Tests for the RemoveContextView.
    ...
    def test_is_importable(self):
        """Test that the RemoveContextView class has successfully been imported, but do not attmept to actually test the class"""
        # Assert the module was loaded
        assert "polychron.views.RemoveContextView" in sys.modules
        # Assert the specific class now exists, but do not instantiate it
        assert isinstance(RemoveContextView, type)
