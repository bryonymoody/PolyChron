...
class TestManageIntrusiveOrResidualContextsView:
    """Tests for the ManageIntrusiveOrResidualContextsView.
    ...
    def test_is_importable(self):
        """Test that the ManageIntrusiveOrResidualContextsView class has successfully been imported, but do not attmept to actually test the class"""
        # Assert the module was loaded
        assert "polychron.views.ManageIntrusiveOrResidualContextsView" in sys.modules
        # Assert the specific class now exists, but do not instantiate it
        assert isinstance(ManageIntrusiveOrResidualContextsView, type)
