...
class TestProjectSelectProcessPopupView:
    """Tests for the ProjectSelectProcessPopupView.
    ...
    def test_is_importable(self):
        """Test that the ProjectSelectProcessPopupView class has successfully been imported, but do not attmept to actually test the class"""
        # Assert the module was loaded
        assert "polychron.views.ProjectSelectProcessPopupView" in sys.modules
        # Assert the specific class now exists, but do not instantiate it
        assert isinstance(ProjectSelectProcessPopupView, type)
