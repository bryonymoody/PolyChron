...
phase_true = 0
calibrate = pd.read_csv('spline_interpolation_new.txt', delim_whitespace=True)    
def trim(im_trim):
   # t0 = time.time()
    """Trims images down"""
...
def chrono_edge_remov(file_graph):
    ...
    if len(phase_list) != 1:
        if len(graph_data[1][3]) == 0:
            print(phase_dic)
            print(phase_dic[phase_list[0]][0], phase_dic[phase_list[1]][0])
...
##change any ';>' to '@>' then back again after
    ...
    print(reversed_dict, [phase_norm, node_list, phase_list, "phase_trck", phase_trck], [x_l, y_l])   
  #  print((sys._getframe().f_code.co_name), str(time.time() - t0))    
...
class MainFrame(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.phaserelbutton = tk.Button(self.canvas, text ='Open phase rel file', command = lambda:open_file5())
        self.phaserelbutton.place(relx=0.45, rely=0.01, relwidth=0.1, relheight=0.03)
        self.eqrelbutton = tk.Button(self.canvas, text ='Open equality file', command = lambda:open_file6())
        ...
        def open_file1(): 
         ...
         #   t0 = time.time()
            ...
            self.text.insert('end', 'Deleted Contexts:\n' + str(self.delnodes)[1:-1])
            self.text.configure(state='normal')
            self.testbutton = tk.Button(self.canvas, text ='Render Chronological DAG', command = lambda:self.chronograph_render()) #popupWindow3(self, self.graph, self.littlecanvas2))
...
#        #canvas to hold the metadata
    ...
    def addedge(self, edgevec):
        ...
        if phase_true == 1:
            imgrender_phase(self.grapmcmc_funch)
        else:
            imgrender(self.graph)
        self.image = Image.open('testdag.png')
        self.show_image()  
...
##        #pt2
    ...
    def MCMC_func(self):
        ...
        TOPO_SORT.reverse()
        mcmc.run_MCMC(calibrate, strat_vec, rcd_est, rcd_err, self.key_ref, context_no, self.popup3.phi_ref, self.popup3.prev_phase, self.popup3.post_phase, TOPO_SORT)
