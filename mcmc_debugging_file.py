...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul 16 15:06:29 2021
...
def strat_rel(site_dict, key, i_index, THETAS, CONTEXT_NO):
    ...
    upstrat = site_dict[key]["dates"][i_index][3][1]
    lowstrat = site_dict[key]["dates"][i_index][3][0]
...
#  tot_i = 0
  while acept_prob > 0:
      ...
      #plot code
          ...
          acept_prob = 0
          POST_S = []
          for m in range(0, 50000):
             # print('theta inits' + str(THETA_INITS))
              print('i' + str(m))
              for k in range(len(THETAS)):
             #    print('k' + str(k))
                  SITE_DICT_TEST_1 = dict_update(SITE_DICT_TEST_1, POST_THETAS, POST_PHIS, m, m, POST_PHASE, PHI_REF)
                 # print(pd.DataFrame.from_dict(SITE_DICT_TEST_1))
                  
                  key = KEY_REF[k]
                  a = [CONTEXT_NO[k] in d for d in SITE_DICT_TEST_1[key]["dates"]].index(True)
                  strat_single = strat_rel(SITE_DICT_TEST_1, key, a, THETAS, CONTEXT_NO)
                  
                  up_bound = max(SITE_DICT_TEST_1[key]["boundaries"][0], strat_single[0])
                  low_bound = min(SITE_DICT_TEST_1[key]["boundaries"][1], strat_single[1])
               #   print('upper and lower bounds' + str([up_bound, low_bound]))
...
for i, j in enumerate(CONTEXT_NO):
    hpd_dict[j] = list(HPD_interval(np.array(ACCEPT[i][1000:15000])))
hpd_df = pd.DataFrame.from_dict(hpd_dict, orient='index')
hpd_df.to_csv("hpd_intervals_thetas_test")    
hpd_dict = {}
for k, l in enumerate(PHI_REF):
    hpd_dict[l] = list(HPD_interval(PHI_ACCEPT[k][1000:15000]))
hpd_df = pd.DataFrame.from_dict(hpd_dict, orient='index')
hpd_df.to_csv("hpd_intervals_phis_test") 
