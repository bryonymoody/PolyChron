...
import os
os.chdir("/home/sm1bmx/Documents/PolyChron/Pythonapp_tests/projects")
...
def polygonfunc(i):
    ...
    if -1*float(a[7]) == -1*float(a[3]):
        coords_converted = [float(a[2]), float(
            a[6]), -1*float(a[5]), -1*float(a[1])]
    else:
        coords_converted = [float(a[2]), float(
            a[6]), -1*float(a[7]), -1*float(a[3])]
...
def ellipsefunc(i):
    ...
    x = x.replace('cy=', '').replace(
        'rx=', '').replace('ry=', '').replace('"', '')
    a = x.split(",")
...
def node_coords_fromjson(graph):
    ...
    scale_info = re.search(
        'points=(.*?)/>', svg_string).group(1).replace(' ', ',')
    scale_info = scale_info.split(",")
    scale = [float(scale_info[4]), -1*float(scale_info[3])]
    coords_x = re.findall(r'id="node(.*?)</text>', svg_string)
    coords_temp = [polygonfunc(
        i) if "points" in i else ellipsefunc(i) for i in coords_x]
...
def chrono_edge_remov(file_graph):
    ...
    if len(phase_list) != 1:
        if len(graph_data[1][3]) == 0:
            file_graph.add_edge(
                phase_dic[phase_list[0]][0], phase_dic[phase_list[1]][0], arrowhead='none')
            graph_data = phase_info_func(file_graph)
        ...
        for i, j in enumerate(olist):
            file_graph.add_edge(
                oddlist[i], "a_" + str(j.replace("_below", "")), arrowhead='none')
    # case when there's only one phase
        ...
        for nd in graph_data[1][1]:
            if len(list(file_graph.predecessors(nd))) == 0:
                # if no predecesors, adds to list for nodes to connect to phase boundary
                evenlist.append(nd)
...
def phase_labels(phi_ref, POST_PHASE, phi_accept, all_samps_phi):
    ...
    for a_val in enumerate(POST_PHASE):
        ...
        if a_val[1] == "abutting":
            labels.append(
                'b_' + str(phi_ref[a_val[0]]) + ' = a_' + str(phi_ref[a_val[0]+1]))
            results_dict['a_' + str(phi_ref[a_val[0]+1]) +
                         ' = b_' + str(phi_ref[a_val[0]])] = phi_accept[i]
            all_results_dict['a_' + str(phi_ref[a_val[0]+1]) +
                             ' = b_' + str(phi_ref[a_val[0]])] = all_samps_phi[i]
           # results_dict['a_' + str(phi_ref[a_val[0]+1])] = phi_accept[i]
            ...
            results_dict['a_' + str(phi_ref[a_val[0]+1])] = phi_accept[i]
            all_results_dict['a_' +
                             str(phi_ref[a_val[0]+1])] = all_samps_phi[i]
        else:
            labels.append('a_' + str(phi_ref[a_val[0]+1]))
            labels.append('b_' + str(phi_ref[a_val[0]]))
            results_dict['a_' + str(phi_ref[a_val[0]+1])] = phi_accept[i]
...
def phase_rels_delete_empty(file_graph, new_phase_rels, p_list, phasedict, phase_nodes, graph_data):
    ...
    [file_graph.add_edge("a_" + str(i[0]), "b_" + str(i[1]),
                         arrowhead='none') for i in new_phase_rels]
    for p in p_list:
        ...
        if relation == 'gap':
            if (p[0] in graph_data[1][2]) == False:
                ...
                file_graph.add_edge(
                    "a_" + str(p[0]), "b_" + str(p[1]), arrowhead='none')
        if relation == 'overlap':
            if (p[0] in graph_data[1][2]) == False:
                null_phases.append(p)
            elif (p[1] in graph_data[1][2]) == False:
                ...
                file_graph.add_edge(
                    "b_" + str(p[1]), "a_" + str(p[0]), arrowhead='none')
        if relation == "abutting":
            if (p[0] in graph_data[1][2]) == False:
                ...
                file_graph = nx.contracted_nodes(
                    file_graph, "a_" + str(p[0]), "b_" + str(p[1]))
                x_nod = list(file_graph)
                newnode = str("a_" + str(p[0]) + " = " + "b_" + str(p[1]))
                label_str = '<&alpha;<SUB>' + \
                    str(p[0]) + '</SUB> = &beta;<SUB>' + str(p[1]) + '</SUB>>'
                label_dict[newnode] = label_str
                phase_nodes.append("a_" + str(p[0]) + " = " + "b_" + str(p[1]))
                y_nod = [newnode if i == "a_" +
                         str(p[0]) else i for i in x_nod]
...
def chrono_edge_add(file_graph, graph_data, xs_ys, phasedict, phase_trck, post_dict, prev_dict):
    ...
    for i in node_list:  # loop adds edges between phases
        if (i in xs) == False:
            if (i in ys) == False:
                file_graph.add_edge(
                    "b_" + str(all_node_phase[i]), i, arrowhead='none')
                file_graph.add_edge(
                    i, "a_" + str(all_node_phase[i]), arrowhead='none')
            else:
                file_graph.add_edge(
                    i, "a_" + str(all_node_phase[i]), arrowhead='none')
        elif (i in xs) == True:
        ...
        for i in enumerate(pset):
            ...
            for j in enumerate(node_list):
                if node_info[j[0]]["fillcolor"] == pset[i[0]]:
                    ...
                    p_phase = str(
                        pset[i[0]][pset[i[0]].rfind("/")+1:len(pset[i[0]])])
                    node_info[j[0]].update({"Group": p_phase})
                mydict[str(pset[i[0]][pset[i[0]].rfind("/") +
                           1:len(pset[i[0]])])] = temp_nodes_list
    else:
        for i in enumerate(pset):
            temp_nodes_list = []
            for j in enumerate(node_list):
        ...
        for key in mydict:
            if test1[i[0]][1] in mydict[key] and test1[i[0]][0] not in mydict[key]:
                ...
                phase_lst = [list(mydict.values()).index(j) for j in list(mydict.values()) if
                             test1[i[0]][0] in j]
                # trying to find phase of other value
                key_1 = (list(mydict.keys())[phase_lst[0]])
...
def phase_info_func(file_graph):
    ...
    if FILE_INPUT != None:
        ...
        phase_norm = [phase[ph][phase[ph].rfind(
            "/")+1:len(phase[ph])] for ph in phase]
# ####code to get colours####
...
def alp_beta_node_add(x, graph):
    ...
    graph.add_node("a_" + str(x), shape="diamond",
                   size="20.0", fontname="helvetica", penwidth="1.0")
    graph.add_node("b_" + str(x), shape="diamond",
                   fontsize="20.0", fontname="helvetica", penwidth="1.0")
...
class popupWindow(object):
    def __init__(self, master):
        ...
        self.l.pack()
        # allows us to keep t6rack of the number we've entered
        self.e = ttk.Entry(self.top)
        self.e.pack()
        # gets ridof the popup
...
#       #defining variables to keep track of what is being updated in the meta data
        ...
        self.optionmenu_a = ttk.OptionMenu(
            self.top, self.variable_a, list(self.dict.keys())[0], *self.dict.keys())
        self.optionmenu_b = ttk.OptionMenu(
            self.top, self.variable_b, 'None', 'None')
...
class popupWindow3(object):
    def __init__(self, master, graph, canvas, phase_rels, dropdown_ns=[], dropdown_intru=[], resid_list=[], intru_list=[]):
        ...
        self.backcanvas.place(relx=0.135, rely=0.9,
                              relheight=0.13, relwidth=0.53)
        self.backcanvas.create_line(150, 30, 600, 30, arrow=tk.LAST,  width=3)
        self.time_label = tk.Label(self.maincanvas, text="Time")
        self.time_label.config(bg='white', font=(
            'helvetica', 12, 'bold'), fg='#2f4858', wraplength=130)
        self.time_label.place(relx=0.32, rely=0.91,
        ...
        datadict = nx.get_node_attributes(
            self.graphcopy, 'Determination')  # get all dates for each notes
        ...
        self.context_no_unordered = [x for x in list(self.graph.nodes(
        )) if x not in self.node_del_tracker]  # sets up a context list
        # set up context types
        self.CONT_TYPE = ['normal' for i in self.context_no_unordered]
        ...
        self.instruc_label = tk.Label(
            self.maincanvas, text="Instructions: \n Place the oldest group in the bottom left corner then for each subseqent group, place it directly above and move it to be overlapping, abutting or to have a gap.")
        self.instruc_label.config(bg='white', font=(
            'helvetica', 12, 'bold'), fg='#2f4858', wraplength=130)
        self.instruc_label.place(relx=0.01, rely=0.05,
                                 relwidth=0.12, relheight=0.85)
        self.instruc_label2 = tk.Label(
            self.maincanvas, text="User defined group relationships")
        self.instruc_label2.config(bg='white', font=(
            'helvetica', 12, 'bold'), fg='#2f4858')
        self.instruc_label2.place(
            relx=0.67, rely=0.17, relwidth=0.32, relheight=0.1)
        ...
        for ind, i in enumerate(self.phase_labels):
            ...
            msg.place(x=0.05*w + (w/(2*m))*ind, y=0.85*h - ((0.95*h)/m)
                      * ind, relwidth=0.76/m, relheight=min(0.1, 0.9/m))
            self.label_dict[i] = msg
        self.button1 = tk.Button(self.maincanvas, text="Confirm groups", command=lambda: self.get_coords(
        ),  bg='#2F4858', font=('Helvetica 12 bold'),  fg='#eff3f6')
        self.button1.place(relx=0.8, rely=0.91)
        self.df = pd.DataFrame(phase_rels, columns=[
        ...
        self.frmtreeborder.place(relx=0.67, rely=0.25,
                                 relheight=0.65, relwidth=0.32)
    ...
    def get_coords(self):
        ...
        self.maincanvas.update()
        self.instruc_label = tk.Label(
            self.maincanvas, text="If you're happy with your group relationships, click the Render Chronological Graph button.")
        self.instruc_label.config(bg='white', font=(
            'helvetica', 12, 'bold'), wraplength=130)
        self.instruc_label.place(relx=0.01, rely=0.05,
                                 relwidth=0.12, relheight=0.85)
        ...
        orig_x_prev = [self.label_dict[j[0]].winfo_x() +
                       ref_w for j in y_final[:-1]]
        self.prev_dict = {}
        self.post_dict = {}
        self.menudict = {}
        for ind, j in enumerate(y_final[1:]):
            ...
            if ind < len(y_final)-1:
                x_prev_curr = self.label_dict[y_final[ind][0]].winfo_x(
                ) + ref_w
                if x - x_prev < -15:
                    x = x_prev_curr - ref_gap
                    self.prev_dict[str(j[0])] = 'overlap'
                    self.post_dict[str(y_final[ind][0])] = 'overlap'
                    self.menudict[(str(j[0]), str(
                        y_final[ind][0]))] = 'overlap'
                    ...
                    self.menudict[(str(j[0]), str(
                        y_final[ind][0]))] = 'abutting'
                # ceter of top box + (half and scalefactor) times height
                y = ref_y - (0.5 + ind + 1)*ref_h
        ...
        for index, row in rels_df.iterrows():
            self.tree.insert("", 0, text=index, values=list(row))
        self.tree['show'] = 'headings'
        self.button_b = tk.Button(self.maincanvas, text='Render Chronological graph', command=lambda: self.full_chronograph_func(
        ), bg='#2F4858', font=('Helvetica 12 bold'),  fg='#eff3f6')
        self.button_b.place(relx=0.75, rely=0.91)
        self.button_a = tk.Button(self.maincanvas, text='Change relationships', command=lambda: self.back_func(
        ), bg='#2F4858', font=('Helvetica 12 bold'),  fg='#eff3f6')
    ...
    def full_chronograph_func(self):
        ...
        if len(self.step_1[0][1][3]) != 0:
            ...
            self.prev_phase.append(
                self.prev_dict[self.phi_ref[len(self.phi_ref)-1]])
        else:
...
# change to new phase rels
        ...
        for i, j in enumerate(self.context_no_unordered):
            ...
            all_paths.extend(nx.all_simple_paths(
                self.graphcopy, source=root, target=leaf))
        ...
        [self.remove_attribute(self.graphcopy, i, "contraction") for i in nodes]
...
class popupWindow4(object):
    def __init__(self, master, controller, resid_list, intru_list, node_track, graph):
        ...
        self.button = tk.Button(self.top, text='Go back', command=lambda: self.top.destroy(
        ),  bg='#2F4858', font=('Helvetica 12 bold'),  fg='#eff3f6')
        self.button.grid(column=30, row=4)
        self.button2 = tk.Button(self.top, text='Proceed to render chronological graph', command=lambda: self.move_to_graph(
        ),  bg='#2F4858', font=('Helvetica 12 bold'),  fg='#eff3f6')
    ...
    def move_to_graph(self):
        ...
        self.popup3 = popupWindow3(startpage, startpage.graph, startpage.littlecanvas2,
    ...
    def test(self, resid_list, intru_list):
        ...
        for i, j in enumerate(resid_list):
            ttk.Label(self.top, text=str(j)).grid(
                column=0, row=i, padx=30, pady=25)
            self.dropdown_ns[j] = tk.StringVar()
            self.dropdowns[j] = ttk.Combobox(
                self.top, width=27, textvariable=self.dropdown_ns[j], state="readonly")
        ...
        for k, l in enumerate(intru_list):
            ttk.Label(self.top, text=str(l)).grid(
                column=21, row=k, padx=30, pady=25)
            self.dropdown_intru[l] = tk.StringVar()
            self.nodetype2[l] = ttk.Combobox(
                self.top, width=27, textvariable=self.dropdown_intru[l], state="readonly")
            # Adding combobox drop down list
...
class popupWindow5(object):
    def __init__(self, master):
        ...
        self.l = tk.Label(top, text="Why are you removing this context from your stratigraphy?",
                          bg='white', font='helvetica 12', fg='#2f4858')
...
class popupWindow6(object):
    def __init__(self, master):
        ...
        self.l = tk.Label(top, text="Why are you deleting the stratigraphic relationship between these contexts?",
                          bg='white', font='helvetica 12', fg='#2f4858')
        self.l.place(relx=0.3, rely=0.1)
        self.e = tk.Text(top, font='helvetica 12', fg='#2f4858')
        self.e.place(relx=0.3, rely=0.2, relheight=0.5, relwidth=0.5)
...
class popupWindow7(object):
    def __init__(self, master, df):
        ...
        for index, row in df.iterrows():
            tree.insert("", 0, text=index, values=list(row))
        tree['show'] = 'headings'
        self.b = tk.Button(top, text='Load data', command=self.cleanup1,
                           bg='#2F4858', font=('Helvetica 12 bold'),  fg='#eff3f6')
        self.b.pack()
        self.c = tk.Button(top, text='Cancel', command=self.cleanup2,
                           bg='#2F4858', font=('Helvetica 12 bold'),  fg='#eff3f6')
        self.c.pack()
...
class popupWindow8(object):
    def __init__(self, master, path):
        self.master = master
        self.path = path
        model_list_prev = [d for d in os.listdir(
            path) if os.path.isdir(path + '/' + d)]
        model_list = []
        ...
        self.top.geometry("1000x400")
        self.l = tk.Label(self.top, text="Which model/s would you like calibrate?",
                          bg='white', font='helvetica 12', fg='#2f4858')
        self.l.place(relx=0.3, rely=0.1)
        self.e = tk.Listbox(self.top, font='helvetica 12',
                            fg='#2f4858', selectmode='multiple')
        self.e.place(relx=0.3, rely=0.2, relheight=0.5, relwidth=0.5)
  #       self.e.bind('<<ListboxSelect>>',tk.CurSelet)
        ...
        self.b = tk.Button(self.top, text='OK', command=self.cleanup,
                           bg='#2F4858', font=('Helvetica 12 bold'),  fg='#eff3f6')
        self.b.place(relx=0.3, rely=0.7)
        self.b = tk.Button(self.top, text='Select all', command=self.selectall,
                           bg='#2F4858', font=('Helvetica 12 bold'),  fg='#eff3f6')
    ...
    def save_state_1(self, j):
        ...
        var_list = [var for var in vars_list_1 if (
            ('__' and 'grid' and 'get' and 'tkinter' and 'children') not in var) and (var[0] != '_')]
        data = {}
...
class popupWindow9(object):
    def __init__(self, master, path):
        ...
        self.master = master
        self.path = path
        model_list_labels = [(str(path) + '/graph_theory_tests_' + str(i), i,
                              'graph_theory_tests_' + str(i)) for i in self.master.graph.nodes()]
  #        model_list = []
        ref_wd = os.getcwd()
        ...
        RCD_EST = [int(list(self.master.datefile["date"])[list(
            self.master.datefile["context"]).index(i)]) for i in base_context_no]
        RCD_ERR = [int(list(self.master.datefile["error"])[list(
            self.master.datefile["context"]).index(i)]) for i in base_context_no]
        ...
        for i, j, k in model_list_labels:
            if k not in os.listdir(path):
                ...
                # remove_node_section
                self.master.chrono_dag = node_del_fixed(
                    self.master.chrono_dag, j)
                self.master.graph = node_del_fixed(self.master.graph, j)
                ref = np.where(np.array(base_context_no) == str(j))[0][0]
                ref2 = np.where(
                    np.array(base_context_no_unordered) == str(j))[0][0]
                phase = self.master.key_ref[ref]
                phase_ref = np.where(
                    np.array(base_phi_ref) == str(phase))[0][0]
            ...
            # change to new phase rels
                ...
                group_conts = [self.master.CONTEXT_NO[i]
                               for i, j in enumerate(self.master.key_ref) if j == phase]
                for m in group_conts:
                    if len(self.master.chrono_dag.out_edges(m)) == 0:
                        alph = [i for i in self.master.chrono_dag.nodes()
                                if "a_" + phase in i]
                        self.master.chrono_dag.add_edge(
                            m, alph[0], arrowhead='none')
                    if len(self.master.chrono_dag.in_edges(m)) == 0:
                        bet = [i for i in self.master.chrono_dag.nodes()
                               if "b_" + phase in i]
                        self.master.chrono_dag.add_edge(
                            bet[0], m, arrowhead='none')
    # setting up the directorys
                dirs4 = self.make_directories(i)
                write_dot(self.master.chrono_dag, 'fi_new_chrono')
                imgrender2(self.master.littlecanvas2.winfo_width(),
                           self.master.littlecanvas2.winfo_height())
                imgrender(self.master.graph, self.master.littlecanvas.winfo_width(
                ), self.master.littlecanvas.winfo_height())
                self.master.ACCEPT = [[]]
    ...
    def prob_overlap(self, ll1, ll2):
        # hellenger distance
        dist_probs = np.array(
            [(np.sqrt(ll2[1][i])-np.sqrt(j))**2 for i, j in enumerate(ll1[1])])
    ...
    def node_importance(self, graph):
        ...
        df_prob_overlap = pd.concat([pd.DataFrame([[str(i), str(j), self.prob_overlap(self.rc_llhds_dict[j], self.rc_llhds_dict[i])]], columns=[
                                    'node', 'neighbour', 'overlap_measure']) for i in G.nodes() for j in list(G.neighbors(i))])
        # katz
        df_prob_overlap.to_csv('overlap_df.csv')
        dd = nx.pagerank(G, alpha=0.85, personalization=None, max_iter=100,
    ...
    def graph_adjust(self, phase, phase_ref):
        ...
        if (phase in self.master.key_ref) == False:
            ...
            if self.master.prev_phase[phase_ref] == 'start':
                phase_node = [
                    i for i in self.master.chrono_dag.nodes() if "b_"+str(phase) in i]
                ...
                phase_node = [
                    i for i in self.master.chrono_dag.nodes() if "a_"+str(phase) in i]
                ...
                phase_nodes = [
                    i for i in self.master.chrono_dag.nodes() if phase in i]
                self.master.chrono_dag.remove_edge(
                    phase_nodes[0], phase_nodes[1])
                self.master.chrono_dag = nx.contracted_nodes(
                    self.master.chrono_dag, phase_nodes[0], phase_nodes[1])
                new_label_1 = str(phase_nodes[0])
                new_label = new_label_1.replace(
                    "a_"+str(phase), "a_" + str(self.master.phi_ref[phase_ref + 1]))
                mapping = {phase_nodes[0]: new_label}
                self.master.prev_phase.pop(phase_ref)
                self.master.post_phase.pop(phase_ref)
                self.master.prev_phase[phase_ref] = 'gap'
                self.master.post_phase[phase_ref] = 'gap'
            self.master.chrono_dag = nx.relabel_nodes(
                self.master.chrono_dag, mapping)
        ...
        for v, w in enumerate(self.master.CONTEXT_NO):
            ...
            if self.master.CONT_TYPE[v] == 'residual':
                ...
                all_paths = []
                all_paths.extend(nx.all_simple_paths(
                    self.master.chrono_dag, source=root, target=leaf))
                for f in all_paths:
    ...
    def save_state_1(self, master, j):
        ...
        if mcmc_check == 'mcmc_loaded':
            ...
            key_ref = [list(phasefile["Group"])[list(
                phasefile["context"]).index(i)] for i in context_no]
...
class popupWindow10(object):
    ...
    #     con_list = list(katz_df_test['context'])
        ...
        for num in range(5, 6):
            ...
            self.master.post_phase = base_post_phase.copy()
            self.master.context_no_unordered = base_context_no_unordered.copy()
            katz_df_test = self.top_nodes(
                num, self.master.phi_ref, self.master.CONTEXT_NO, self.master.key_ref, path)
            # remove_node_section
            remove_conts = [
                i for i in self.master.CONTEXT_NO if i not in katz_df_test]
            ...
            for j in remove_conts:
                ...
                ref = np.where(np.array(self.master.CONTEXT_NO)
                               == str(j))[0][0]
                ref2 = np.where(
                    np.array(self.master.context_no_unordered) == str(j))[0][0]
                phase = self.master.key_ref[ref]
                phase_ref = np.where(
                    np.array(base_phi_ref) == str(phase))[0][0]
                self.master.CONT_TYPE.pop(ref)
    ...
    def katz_plus_overlap(self, path, refmodel, mode='strat'):
        if mode == 'strat':
            ...
            katz_df_test = pd.read_csv(
                path + "/" + refmodel + "/katz_centr_chrono.csv")
        katz_df_test = katz_df_test[['context', 'pagerank']]
        katz_df_test['context'] = katz_df_test['context'].astype(str)
        katz_df_test = katz_df_test.loc[(
            katz_df_test["context"].str.contains("a") == False)]
        katz_df_test = katz_df_test.loc[(
            katz_df_test["context"].str.contains("b") == False)]
        katz_df_test = katz_df_test.transpose()
        katz_df = katz_df_test.rename(columns=katz_df_test.iloc[0]).drop(
            katz_df_test.index[0]).reset_index(drop=True)
    ...
    def katz_w_weight(self, path):
        ...
        katz_df, ll_over_df = self.katz_plus_overlap(
            path, 'reference_model', mode='chrono')
        ll_over_df['neighbour'] = ll_over_df['neighbour'].astype(str)
        ll_over_df['node'] = ll_over_df['node'].astype(str)
        for v, w in ref_graph.edges:
            ref_graph.edges[v, w]["weight"] = float(ll_over_df.loc[(
                ll_over_df['node'] == v) & (ll_over_df['neighbour'] == w)]['overlap_measure'])
    ...
    def top_nodes(self, num, phi_ref, context_no, key_ref, path):
        ...
        katz_vals = [weighted_katz[label]
                     for i, label in enumerate(context_no)]
        df = pd.DataFrame()
...
class load_Window(object):
    ...
    def initscreen(self):
        [x.destroy() for x in [self.greeting, self.b, self.c, self.back, self.back1,
                               self.l, self.MyListBox, self.text_1, self.user_input] if x != None]
        self.maincanvas.update()
        image1 = Image.open(
            "/home/sm1bmx/Documents/PolyChron/Pythonapp_tests/projects/logo.png")
 ...
 #       self.imagetk2 = ImageTk.PhotoImage(image2.resize((int(x_2 - x_1), int(y_2 - y_1))))
        ...
        self.greeting = tk.Label(self.maincanvas, text="Welcome to PolyChron, how would you like to proceed?",
                                 bg='white', font=('helvetica 12 bold'), fg='#2F4858', anchor='w')
        self.greeting.place(relx=0.22, rely=0.45)
        self.b = tk.Button(self.maincanvas, text='Load existing project', command=lambda: self.load_proj(
        ), bg='#2F4858', font=('Helvetica 12 bold'),  fg='#eff3f6')
        self.b.place(relx=0.8, rely=0.9)
        self.c = tk.Button(self.maincanvas, text='Create new project', command=lambda: self.new_proj(
        ), bg='#eff3f6', font=('Helvetica 12 bold'),  fg='#2F4858')
    ...
    def load_proj(self):
        global proj_dir
        os.chdir("/home/sm1bmx/Documents/PolyChron/Pythonapp_tests/projects")
        [x.destroy() for x in [self.label1, self.greeting, self.b, self.c, self.back,
                               self.l, self.back1, self.MyListBox, self.text_1, self.user_input] if x != None]
        self.maincanvas.update()

        self.l = tk.Label(self.maincanvas, text="Select project",
                          bg='white', font=('helvetica 14 bold'), fg='#2F4858')
        self.l.place(relx=0.36, rely=0.1)
        myList = [d for d in os.listdir(
            "/home/sm1bmx/Documents/PolyChron/Pythonapp_tests/projects") if os.path.isdir(d)]
        myList = [d for d in myList if (d != '__pycache__') and (d != 'Data')]
        mylist_var = tk.StringVar(value=myList)
        self.MyListBox = tk.Listbox(self.maincanvas, listvariable=mylist_var, bg='#eff3f6', font=(
            'Helvetica 11 bold'),  fg='#2F4858', selectmode='browse')
        scrollbar = ttk.Scrollbar(
            self.maincanvas,
            orient='vertical',
            command=self.MyListBox.yview)
        self.MyListBox['yscrollcommand'] = scrollbar.set
        self.MyListBox.place(relx=0.36, rely=0.17,
                             relheight=0.4, relwidth=0.28)
        self.MyListBox.bind('<<ListboxSelect>>', self.items_selected)
        self.b = tk.Button(self.maincanvas, text='Load project', command=lambda: self.load_model(
            proj_dir),  bg='#2F4858', font=('Helvetica 12 bold'),  fg='#eff3f6')
        self.b.place(relx=0.8, rely=0.9, relwidth=0.19)
        self.top.bind('<Return>', (lambda event: self.load_model(proj_dir)))
        self.back = tk.Button(self.maincanvas, text='Back', command=lambda: self.initscreen(
        ),  bg='#eff3f6', font=('Helvetica 12 bold'),  fg='#2F4858')
        self.back.place(relx=0.21, rely=0.01)

    def load_model(self, direc):
        global proj_dir
        [x.destroy() for x in [self.greeting, self.label1, self.b, self.c,
                               self.back, self.back1, self.l, self.MyListBox] if x != None]
        ...
        self.l = tk.Label(self.maincanvas, text="Model list",
                          bg='white', font=('helvetica 14 bold'), fg='#2F4858')
        self.l.place(relx=0.36, rely=0.1)
       # myList_all = os.listdir("/home/bryony/Documents/Pythonapp_tests/projects")
        ...
        self.MyListBox = tk.Listbox(self.maincanvas, listvariable=self.model_list, bg='#eff3f6', font=(
            'Helvetica 11 bold'),  fg='#2F4858', selectmode='browse')
        scrollbar = ttk.Scrollbar(
            self.maincanvas,
            orient='vertical',
        ...
        self.MyListBox.place(relx=0.36, rely=0.17,
                             relheight=0.4, relwidth=0.28)
        self.MyListBox.bind('<<ListboxSelect>>', self.items_selected)
        self.b = tk.Button(self.maincanvas, text='Load selected model', command=lambda: self.cleanup2(
        ),  bg='#2F4858', font=('Helvetica 12 bold'),  fg='#eff3f6')
        ...
        self.back.place(relx=0.21, rely=0.01)
        self.back1 = tk.Button(self.maincanvas, text='Create new model', command=lambda: self.new_model(
            path),  bg='#eff3f6', font=('Helvetica 12 bold'),  fg='#2F4858')
    ...
    def items_selected(self, event):
        ...
        # get selected items
        self.selected_langs = ",".join(
            [self.MyListBox.get(i) for i in selected_indices])

    def create_file(self, folder_dir, load):
        global proj_dir
        dirs = os.path.join(
            "/home/sm1bmx/Documents/PolyChron/Pythonapp_tests/projects", folder_dir, self.model.get())
        ...
        if not os.path.exists(dirs):
           #     os.makedirs(dirs)
            ...
            proj_dir = os.path.join(
                "/home/sm1bmx/Documents/PolyChron/Pythonapp_tests/projects", folder_dir)
            if load == True:
                for F in (StartPage, PageOne):
                    page_name = F.__name__
                    frame = F(parent=self.master.container,
            ...
            tk.messagebox.showerror(
                'Tips', 'The folder name exists, please change it')
            self.cleanup()

    def new_proj(self):
        [x.destroy() for x in [self.greeting, self.label1, self.b, self.back1, self.c,
                               self.back, self.l, self.MyListBox, self.text_1, self.user_input] if x != None]
        self.maincanvas.update()
        self.folder = tk.StringVar()  # Receiving user's folder_name selection
        self.text_1 = tk.Label(self.maincanvas, text="Input project name:", bg='white', font=(
            'helvetica 14 bold'), fg='#2F4858')
        self.text_1.place(relx=0.4, rely=0.2)
        self.user_input = tk.Entry(self.maincanvas, textvariable=self.folder)
        self.user_input.place(relx=0.35, rely=0.4,
                              relwidth=0.3, relheight=0.08)
        self.b = tk.Button(self.maincanvas, text="Submit ", command=lambda: self.new_model(
            self.folder.get()), bg='#ec9949', font=('Helvetica 12 bold'),  fg='#2F4858')
        self.top.bind(
            '<Return>', (lambda event: self.new_model(self.folder.get())))
        self.b.place(relx=0.66, rely=0.4)
        self.back = tk.Button(self.maincanvas, text='Back', command=lambda: self.initscreen(
        ), bg='#dcdcdc', font=('helvetica 12 bold'), fg='#2F4858')
        self.back.place(relx=0.21, rely=0.01)

    def new_model(self, folder_dir, load=True):
        [x.destroy() for x in [self.greeting, self.label1, self.b, self.c, self.back,
                               self.back1, self.l, self.MyListBox, self.text_1, self.user_input] if x != None]
        self.maincanvas.update()
        self.model = tk.StringVar()  # Receiving user's folder_name selection
        self.text_1 = tk.Label(self.maincanvas, text="Input model name:", bg='white', font=(
            'helvetica 14 bold'), fg='#2F4858')
        self.text_1.place(relx=0.4, rely=0.2)
        self.user_input = tk.Entry(self.maincanvas, textvariable=self.model)
        self.user_input.place(relx=0.35, rely=0.4,
                              relwidth=0.3, relheight=0.08)
        self.b = tk.Button(self.maincanvas, text="Submit ", command=lambda: self.create_file(
            folder_dir, load), bg='#ec9949', font=('Helvetica 12 bold'),  fg='#2F4858')
        self.b.place(relx=0.66, rely=0.4)
        self.top.bind(
            '<Return>', (lambda event:  self.create_file(folder_dir, load)))
        self.back = tk.Button(self.maincanvas, text='Back', command=lambda: self.new_proj(
        ), bg='#dcdcdc', font=('helvetica 12 bold'), fg='#2F4858')
        self.back.place(relx=0.21, rely=0.01)
        return self.top
...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.button1_a.place(relx=0.55, rely=0.0,
        ...
        file = tk.Menu(self.file_menubar, tearoff=0,
                       bg='white', font=("helvetica 12 bold"))
        self.file_menubar["menu"] = file
        file.add_separator()
        FILE_INPUT = None
        file.add_command(label='Load stratigraphic diagram file (.dot)',
                         command=lambda: self.open_file1(), font="helvetica 12 bold")
        file.add_command(label='Load stratigraphic relationship file (.csv)',
                         command=lambda: self.open_file2(), font="helvetica 12 bold")
        file.add_command(label='Load scientific dating file (.csv)',
                         command=lambda: self.open_file3(), font="helvetica 12 bold")
        file.add_command(label='Load context grouping file (.csv)',
                         command=lambda: self.open_file4(), font="helvetica 12 bold")
        file.add_command(label='Load group relationship file (.csv)',
                         command=lambda: self.open_file5(), font="helvetica 12 bold")
        file.add_command(label='Load context equalities file (.csv)',
                         command=lambda: self.open_file6(), font="helvetica 12 bold")
        file.add_command(label='Load new project', command=lambda: load_Window(
            MAIN_FRAME), font="helvetica 12 bold")
        file.add_command(label='Load existing model', command=lambda: load_Window.load_model(
            load_Window(MAIN_FRAME), proj_dir), font="helvetica 12 bold")
        file.add_command(label='Save changes as current model',
                         command=lambda: self.save_state_1(), font="helvetica 12 bold")
        file.add_command(label='Save changes as new model', command=lambda: self.refresh_4_new_model(
            controller, proj_dir, load=False), font="helvetica 12 bold")
        file.add_separator()
        file.add_command(label='Exit', command=lambda: self.destroy1)
        self.file_menubar.place(
            relx=0.00, rely=0, relwidth=0.1, relheight=0.03)
        self.view_menubar = ttk.Menubutton(self, text='View')
        # Adding File Menu and commands
        file1 = tk.Menu(self.view_menubar, tearoff=0,
                        bg='white', font=('helvetica', 11))
        self.view_menubar["menu"] = file1
        file1.add_command(label='Display Stratigraphic diagram in phases',
                          command=lambda: self.phasing(), font='helvetica 11')
        self.view_menubar.place(
            relx=0.07, rely=0, relwidth=0.1, relheight=0.03)

        self.tool_menubar = ttk.Menubutton(self, text='Tools')
        # Adding File Menu and commands
        file2 = tk.Menu(self.tool_menubar, tearoff=0,
                        bg='white', font=('helvetica', 11))
        self.tool_menubar["menu"] = file2
        # file2.add_separator()
        file2.add_command(label='Render chronological graph',
                          command=lambda: self.chronograph_render_wrap(), font='helvetica 12 bold')
        file2.add_command(label='Calibrate model',
                          command=lambda: self.load_mcmc(), font='helvetica 12 bold')
        file2.add_command(label='Calibrate multiple projects from project',
                          command=lambda: popupWindow8(self, proj_dir), font='helvetica 12 bold')
        file2.add_command(label='Calibrate node delete variations (alpha)',
                          command=lambda: popupWindow9(self, proj_dir), font='helvetica 12 bold')
        file2.add_command(label='Calibrate important variations (alpha)',
                          command=lambda: popupWindow10(self, proj_dir), font='helvetica 12 bold')

        # file2.add_separator()
        self.tool_menubar.place(
            relx=0.14, rely=0, relwidth=0.1, relheight=0.03)
        #############################
        self.behindcanvas = tk.Canvas(
            self.canvas, bd=0, highlightthickness=0, bg='#33658a')
        self.behindcanvas.place(relx=0.003, rely=0.038,
                                relwidth=0.37, relheight=0.96)
        ############################
        self.behindcanvas2 = tk.Canvas(
            self.canvas, bd=0, highlightthickness=0, bg='#33658a')
        self.behindcanvas2.place(
            relx=0.38, rely=0.038, relwidth=0.37, relheight=0.96)
        ######################
        self.labelcanvas = tk.Canvas(
            self.canvas, bd=0, highlightthickness=0, bg='#33658a')
        self.labelcanvas.place(relx=0.003, rely=0.011,
                               relwidth=0.18, relheight=0.027)
        self.labelcanvas1_id = self.labelcanvas.create_text(
            10, 3, anchor="nw", fill='white')
        self.labelcanvas.itemconfig(
            self.labelcanvas1_id, text="Stratigraphic graph", font='helvetica 12 bold')
        #########################
        ...
        self.littlecanvas_id = self.littlecanvas.create_text(
            10, 10, anchor="nw", fill='#2f4845')
        self.littlecanvas.place(relx=0.005, rely=0.005,
                                relwidth=0.99, relheight=0.99)
        self.littlecanvas.itemconfig(
            self.littlecanvas_id, text="No stratigraphic graph loaded. \n \n \nTo load, go to File > Load stratigraphic diagram",  font='helvetica 12 bold')
        ...
        self.littlecanvas2.itemconfig(
            self.littlecanvas2_id, text="No chronological graph loaded. \n \n \nYou must load a stratigraphic graph first. \nTo load, go to File > Load stratigraphic diagram \nTo load your chronological graph, go to Tools > Render chronological graph",  font='helvetica 12 bold')
        self.littlecanvas2.place(
            relx=0.005, rely=0.005, relwidth=0.99, relheight=0.99)
        ##########################
        self.labelcanvas2 = tk.Canvas(
            self.canvas, bd=0, highlightthickness=0, bg='#33658a')
        self.labelcanvas2.place(relx=0.38, rely=0.011,
                                relwidth=0.18, relheight=0.027)
        self.labelcanvas2_id = self.labelcanvas2.create_text(
            10, 3, anchor="nw", fill='white')
        self.labelcanvas2.itemconfig(
            self.labelcanvas2_id, text="Chronological graph", font='helvetica 12 bold')
        ###################
        self.littlecanvas.bind("<MouseWheel>", self.wheel)
        # only with Linux, wheel scroll down
        self.littlecanvas.bind('<Button-4>', self.wheel)
        ...
        # only with Linux, wheel scroll down
        self.littlecanvas2.bind('<Button-4>', self.wheel2)
...
# meta data table
        self.labelcanvas3 = tk.Canvas(
            self.canvas, bd=0, highlightthickness=0, bg='#33658a')
        self.labelcanvas3.place(relx=0.755, rely=0.695,
                                relwidth=0.17, relheight=0.029)
        self.behindcanvas3 = tk.Canvas(
            self.canvas, bd=0, highlightthickness=0, bg='#33658a')
        self.behindcanvas3.place(
            relx=0.755, rely=0.719, relwidth=0.23, relheight=0.278)
        self.metatext_id = self.labelcanvas3.create_text(
            10, 5, anchor="nw", fill='white')
        self.labelcanvas3.itemconfig(
            self.metatext_id, text="Supplementary data",  font='helvetica 12 bold')
...
# deleted contexts table
        self.labelcanvas4 = tk.Canvas(
            self.canvas, bd=0, highlightthickness=0, bg='#33658a')
        self.labelcanvas4.place(relx=0.755, rely=0.04,
                                relwidth=0.17, relheight=0.029)
        self.behindcanvas4 = tk.Canvas(
            self.canvas, bd=0, highlightthickness=0, bg='#33658a')
        self.behindcanvas4.place(
            relx=0.755, rely=0.069, relwidth=0.23, relheight=0.278)
        self.delcontext_id = self.labelcanvas4.create_text(
            10, 5, anchor="nw", fill='white')
        self.labelcanvas4.itemconfig(
            self.delcontext_id, text="Deleted Contexts",  font='helvetica 12 bold')
...
# deleted edges table
        self.labelcanvas5 = tk.Canvas(
            self.canvas, bd=0, highlightthickness=0, bg='#33658a')
        self.labelcanvas5.place(relx=0.755, rely=0.371,
                                relwidth=0.17, relheight=0.029)
        self.behindcanvas5 = tk.Canvas(
            self.canvas, bd=0, highlightthickness=0, bg='#33658a')
        self.behindcanvas5.place(
            relx=0.755, rely=0.399, relwidth=0.23, relheight=0.278)
        self.deledge_id = self.labelcanvas5.create_text(
            10, 5, anchor="nw", fill='white')
        ...
        self.databutton.place(relx=0.303, rely=0.04,
                              relwidth=0.07, relheight=0.028)
        self.datacanvas = tk.Canvas(
            self.behindcanvas, bd=0, highlightthickness=0, bg='#33658a')
        self.datacanvas.place(relx=0.55, rely=0.0,
        ...
        self.datalittlecanvas.place(
            relx=0.015, rely=0.015, relwidth=0.97, relheight=0.97)
        self.display_data_var = 'hidden'
        self.check_list_gen()
        tk.Misc.lift(self.littlecanvas)

    def refresh_4_new_model(self, controller, proj_dir, load):
        extra_top = load_Window.new_model(
            load_Window(MAIN_FRAME), proj_dir, load)
 ...
 #       self.save_state_1()
    ...
    def check_list_gen(self):
        ...
        self.datalittlecanvas.pack()
        self.datalittlecanvas.create_text(
            10, 110, anchor='nw', text=rels, font=('Helvetica 12 bold'), fill=col4)
        self.datalittlecanvas.pack()
    ...
    def resid_check(self):
        ...
        MsgBox = tk.messagebox.askquestion(
            'Residual and Intrusive Contexts', 'Do you suspect any of your samples are residual or intrusive?', icon='warning')
        if MsgBox == 'yes':
            ...
            self.popup3 = popupWindow3(
                self, self.graph, self.littlecanvas2, self.phase_rels)
    ...
    def save_state_1(self):
        ...
        for child in self.tree2.get_children():
            row_list.append(
                (self.tree2.item(child)['text'], self.tree2.item(child)['values']))
  ...
  #      self.node_importance(self.graph)
    ...
    def restore_state(self):
        ...
        if self.graph is not None:
            ...
            for i, j in enumerate(self.treeview_df['context']):
                self.tree2.insert(
                    "", 'end', text=j, values=self.treeview_df['Reason for deleting'][i])
        ...
        #  self.imscale2 = 1.0  # scale for the canvaas image
                ...
                self.container2 = self.littlecanvas2.create_rectangle(
                    0, 0, self.width2, self.height2, width=0)
                self.imscale2 = min(
                    921/self.image2.size[0], 702/self.image2.size[1])
                # rescale all canvas objects
                self.littlecanvas.scale('all', 0, 0, self.delta2, self.delta2)
                self.show_image2()
    ...
    def onRight(self, *args):
        ...
        self.cursorx = int(self.littlecanvas.winfo_pointerx() -
                           self.littlecanvas.winfo_rootx())
        self.cursory = int(self.littlecanvas.winfo_pointery() -
                           self.littlecanvas.winfo_rooty())
    ...
    #  self.imscale  = min(921/self.image.size[0], 702/self.image.size[1])
        ...
        # Put image into container rectangle and use it to set proper coordinates to the image
        self.container = self.littlecanvas.create_rectangle(
            0, 0, self.width, self.height, width=0)
        self.bind("<Configure>", self.resize)
        # rescale all canvas objects
        self.littlecanvas.scale('all', 0, 0, self.delta, self.delta)
        self.show_image()
 ...
 #       self.image = self.image_ws.resize((int(self.image_ws.size[0]*scale_factor), int(self.image_ws.size[1]*scale_factor)), Image.ANTIALIAS)
    ...
    def chronograph_render_wrap(self):
        ...
        if (self.phase_rels is None) or (self.phasefile is None) or (self.datefile is None):
            tk.messagebox.showinfo(
                "Error", "You haven't loaded in all the data required for a chronological graph")
        if load_check == "loaded":
            answer = askquestion(
                'Warning!', 'Chronological DAG already loaded, are you sure you want to write over it? You can copy this model in the file menu if you want to consider multiple models')
            if answer == 'yes':
            ...
            try:
                ...
                if load_it == 'load':
                    ...
                    for i in set(node_set):
                        G.add_node(i, shape="box", fontname="helvetica",
                                   fontsize="30.0", penwidth="1.0", color='black')
                   ...
                   #     self.imscale = 1.0#, self.littlecanvas.winfo_height()/self.image.size[1])# scale for the canvaas image
                        ...
                        self.littlecanvas.bind("<Button-3>", self.preClick)
                        self.imscale = min(
                            921/self.image.size[0], 702/self.image.size[1])
                        # rescale all canvas objects
                        self.littlecanvas.scale(
                            'all', 0, 0, self.delta, self.delta)
                        self.show_image()
                    tk.messagebox.showinfo(
                        "Success", "Stratigraphic data loaded")
                    self.check_list_gen()
            except ValueError:
                tk.messagebox.showerror(
                    "showerror", "Data not loaded, please try again")
    ...
    def open_file3(self):
        ...
        if file is not None:
            try:
                ...
                self.datefile = self.datefile.map(str)
                ...
                self.check_list_gen()
                tk.messagebox.showinfo(
                    "Success", "Scientific dating data loaded")
            except ValueError:
                tk.messagebox.showerror(
                    "showerror", "Data not loaded, please try again")
    ...
    def open_file5(self):
        ...
        if file is not None:
            try:
                ...
                self.phase_rels = [(str(phase_rel_df['above'][i]), str(
                    phase_rel_df['below'][i])) for i in range(len(phase_rel_df))]
                self.file_popup(pd.DataFrame(self.phase_rels, columns=[
                                'Younger group', 'Older group']))
                self.phase_rel_check = True
                self.check_list_gen()
                tk.messagebox.showinfo(
                    "Success", "Group relationships data loaded")
            except ValueError:
                tk.messagebox.showerror(
                    "showerror", "Data not loaded, please try again")
    ...
    def open_file6(self):
        ...
        if file is not None:
            try:
                ...
                self.equal_rel_df = equal_rel_df.map(str)
                ...
                if phase_true == 1:
                    ...
                    imgrender(self.graph, self.littlecanvas.winfo_width(),
                              self.littlecanvas.winfo_height())
    ...
    def load_mcmc(self):
        ...
        self.l = tk.Label(self.backcanvas, text="MCMC in progress", font=(
            'helvetica 14 bold'), fg='#2F4858', bg='#AEC7D6')
        self.l.place(relx=0.35, rely=0.26)
        outputPanel = tk.Label(self.backcanvas, font=(
            'helvetica 14 bold'), fg='#2F4858', bg='#AEC7D6')
        outputPanel.place(relx=0.4, rely=0.4)
        pb1 = ttk.Progressbar(
            self.backcanvas, orient=tk.HORIZONTAL, length=400, mode='indeterminate')
    ...
    def addedge(self, edgevec):
        ...
        if self.graph.edges() != self.graph_check.edges():
            ...
            tk.messagebox.showerror(
                "Redundant relationship", "That stratigraphic relationship is already implied by other relationships in the graph")
        if phase_true == 1:
            imgrender_phase(self.graph)
        else:
            imgrender(self.graph, self.littlecanvas.winfo_width(),
    ...
    def chronograph_render(self):
        ...
        if load_check != 'loaded':
            ...
            self.image2 = imgrender2(
                self.littlecanvas2.winfo_width(), self.littlecanvas2.winfo_height())
            if self.image2 != 'No_image':
                try:
                    self.littlecanvas2.delete('all')
                    self.littlecanvas2.img = ImageTk.PhotoImage(self.image2)
                    ...
                    self.imscale2 = min(
                        921/self.image2.size[0], 702/self.image2.size[1])
                    # rescale all canvas objects
                    self.littlecanvas2.scale(
                        'all', 0, 0, self.delta2, self.delta2)
                    self.show_image2()
                    self.littlecanvas2.bind("<Configure>", self.resize2)
    ...
    def MCMC_func(self):
        ...
        context_no = [x for x in list(
            self.context_no_unordered) if x not in self.node_del_tracker]
        TOPO = list(nx.topological_sort(self.chrono_dag))
        self.TOPO_SORT = [x for x in TOPO if (
            x not in self.node_del_tracker) and (x in context_no)]
        self.TOPO_SORT.reverse()
        context_no = self.TOPO_SORT
        self.key_ref = [list(self.phasefile["Group"])[list(
            self.phasefile["context"]).index(i)] for i in context_no]
        self.CONT_TYPE = [self.CONT_TYPE[list(
            self.context_no_unordered).index(i)] for i in self.TOPO_SORT]
        strat_vec = []
        resids = [j for i, j in enumerate(
            context_no) if self.CONT_TYPE[i] == 'residual']
        intrus = [j for i, j in enumerate(
            context_no) if self.CONT_TYPE[i] == 'intrusive']
        ...
        self.RCD_EST = [int(list(self.datefile["date"])[list(
            self.datefile["context"]).index(i)]) for i in context_no]
        self.RCD_ERR = [int(list(self.datefile["error"])[list(
            self.datefile["context"]).index(i)]) for i in context_no]
        rcd_est = self.RCD_EST
        rcd_err = self.RCD_ERR
        self.prev_phase, self.post_phase = self.prev_phase, self.post_phase
        input_1 = [strat_vec, rcd_est, rcd_err, self.key_ref, context_no, self.phi_ref,
                   self.prev_phase, self.post_phase, self.TOPO_SORT, self.CONT_TYPE]
        f = open('input_file', 'w')
        writer = csv.writer(f)
    #  for i in input_1:
        ...
        CONTEXT_NO, ACCEPT, PHI_ACCEPT, PHI_REF, A, P, ALL_SAMPS_CONT, ALL_SAMPS_PHI = mcmc.run_MCMC(
            CALIBRATION, strat_vec, rcd_est, rcd_err, self.key_ref, context_no, self.phi_ref, self.prev_phase, self.post_phase, self.TOPO_SORT, self.CONT_TYPE)
        phase_nodes, resultsdict, all_results_dict = phase_labels(
            PHI_REF, self.post_phase, PHI_ACCEPT, ALL_SAMPS_PHI)
        for i, j in enumerate(CONTEXT_NO):
        ...
        if self.variable.get() == "Delete context":
            if self.node != "no node":
                if load_check == 'loaded':
                    load_check = 'not_loaded'
                    answer = askquestion(
                        'Warning!', 'Chronological DAG already loaded, do you want to save this as a new model first? \n\n Click Yes to save as new model and No to overwrite existing model')
                    if answer == 'yes':
                        self.refresh_4_new_model(
                            self.controller, proj_dir, load=False)
                    self.littlecanvas2.delete('all')
            #   self.graph.remove_node(self.node)
                ...
                self.tree2.insert("", 'end', text=self.node,
                                  values=[self.nodedel_meta])
       ...
       # checks if any nodes are in edge node to see if we want to add/delete a context
        if len(self.edge_nodes) == 1:
            ...
            if self.variable.get() == "Delete stratigraphic relationship with " + str(self.edge_nodes[0]):
                ...
                try:
                    self.graph.remove_edge(
                        self.edge_nodes[0], self.edge_nodes[1])
                    self.edge_render()
                    self.tree3.insert("", 0, text=self.temp,
                                      values=self.reason)
                    self.tree3.update()
                except (KeyError, nx.exception.NetworkXError):
                    try:
                        self.graph.remove_edge(
                            self.edge_nodes[1], self.edge_nodes[0])
                        self.edge_render()
                    except (KeyError, nx.exception.NetworkXError):
                        tk.messagebox.showinfo(
                            'Error', 'An edge doesnt exist between those nodes')
            ...
            # second case is adding a strat relationship
                ...
                self.OptionList.remove(
                    "Place " + str(self.edge_nodes[0]) + " Above")
                self.testmenu = ttk.OptionMenu(
                    self.littlecanvas, self.variable, self.OptionList[0], *self.OptionList, command=self.nodes)
                self.edge_nodes = []
        ...
        if len(self.comb_nodes) == 1:
            if self.variable.get() == "Equate context with " + str(self.comb_nodes[0]):
                ...
                graph_temp = nx.contracted_nodes(
                    self.graph, self.comb_nodes[0], self.comb_nodes[1])
                x_nod = list(graph_temp)
                newnode = str(self.comb_nodes[0]) + \
                    " = " + str(self.comb_nodes[1])
                y_nod = [newnode if i == self.comb_nodes[0]
                         else i for i in x_nod]
                mapping = dict(zip(x_nod, y_nod))
                ...
                try:
                    ...
                    if e.__class__.__name__ == 'NetworkXError':
                        tk.messagebox.showinfo(
                            'Error!', 'This creates a cycle so you cannot equate these contexts')
                self.OptionList.remove(
                    "Equate context with " + str(self.comb_nodes[0]))
                self.testmenu = ttk.OptionMenu(
                    self.littlecanvas, self.variable, self.OptionList[0], *self.OptionList, command=self.nodes)
                self.comb_nodes = []
        ...
        if self.variable.get() == "Get supplementary data for this context":
            ...
            if self.meta.loc["Determination"][0] != "None":
                self.meta.loc["Determination"][0] = str(
                    self.meta.loc["Determination"][0][0]) + " +- " + str(self.meta.loc["Determination"][0][1]) + " Carbon BP"
    ...
    def wheel(self, event):
        ...
        if event.num == 4 or event.delta == 120:  # scroll up
            i = min(self.littlecanvas.winfo_width(),
                    self.littlecanvas.winfo_height())
            if i < self.imscale:
    ...
    #    print(scale)
        # rescale all canvas objects
        self.littlecanvas.scale('all', 0, 0, scale, scale)
        self.show_image()
    ...
    def wheel2(self, event):
        ...
        if event.num == 5 or event.delta == -120:  # scroll down
            ...
            scale2 /= self.delta2
        if event.num == 4 or event.delta == 120:  # scroll up
            i = min(self.littlecanvas2.winfo_width(),
                    self.littlecanvas2.winfo_height())
            if i < self.imscale2:
                return  # 1 pixel is bigger than the visible area
            self.imscale2 *= self.delta2
            scale2 *= self.delta2
        # rescale all canvas objects
        self.littlecanvas2.scale('all', 0, 0, scale2, scale2)
        self.show_image2()

    def show_image(self):
        """Show image on the Canvas"""

        bbox1 = [0, 0, int(self.image.size[0]*self.imscale),
                 int(self.image.size[1]*self.imscale)]
        ...
        if int(x_2 - x_1) > 0 and int(y_2 - y_1) > 0:  # show image if it in the visible area
            ...
            x_img = min(int(x_2 / self.imscale), self.width)
            # ...and sometimes not
            y_img = min(int(y_2 / self.imscale), self.height)
            image = self.image.crop((int(x_1 / self.imscale), int(y_1 / self.imscale),
                                     x_img, y_img))
            self.imagetk = ImageTk.PhotoImage(
                image.resize((int(x_2 - x_1), int(y_2 - y_1))))
    ...
    def show_image2(self):
        """Show image on the Canvas"""
        bbox1 = [0, 0, int(self.image2.size[0]*self.imscale2),
                 int(self.image2.size[1]*self.imscale2)]
        # Remove 1 pixel shift at the sides of the bbox1
        bbox1 = (bbox1[0] + 1, bbox1[1] + 1, bbox1[2] - 1, bbox1[3] - 1)
        ...
        if int(x_2 - x_1) > 0 and int(y_2 - y_1) > 0:  # show image if it in the visible area
            # sometimes it is larger on 1 pixel
            x_img = min(int(x_2 / self.imscale2), self.width2)
            # ...and sometimes not
            y_img = min(int(y_2 / self.imscale2), self.height2)
            image2 = self.image2.crop((int(x_1 / self.imscale2), int(y_1 / self.imscale2),
                                       x_img, y_img))
            self.imagetk2 = ImageTk.PhotoImage(
                image2.resize((int(x_2 - x_1), int(y_2 - y_1))))
    ...
    def resize(self, event):
        ...
        img = Image.open(
            # .resize((event.width, event.height), Image.ANTIALIAS)
            'testdag.png')
        self.littlecanvas.img = ImageTk.PhotoImage(img)
        self.w_1 = event.width
        self.h_1 = event.height
        self.littlecanvas.itemconfig(
            self.littlecanvas_img, image=self.littlecanvas.img)
    ...
    def resize2(self, event):
        ...
        self.h_1 = event.height
        self.littlecanvas2.itemconfig(
            self.littlecanvas2_img, image=self.littlecanvas2.img)
...
#        self.metatext = ""
        ...
        # Adding File Menu and commands
        # , font = ('helvetica',11))
        file = tk.Menu(self.file_menubar, tearoff=0, bg='#fcfdfd')
        self.file_menubar["menu"] = file
        file.add_separator()
        file.add_command(label='Save project progress',
                         command=lambda: startpage.save_state_1(), font=('helvetica 11 bold'))
        self.file_menubar.place(relx=0.0, rely=0, relwidth=0.1, relheight=0.03)
        self.view_menubar = ttk.Menubutton(self, text='View')
        # Adding File Menu and commands
        # , font = ('helvetica',11))
        file1 = tk.Menu(self.view_menubar, tearoff=0, bg='#fcfdfd')
        self.view_menubar["menu"] = file1
        self.view_menubar.place(
            relx=0.07, rely=0, relwidth=0.1, relheight=0.03)
        self.tool_menubar = ttk.Menubutton(self, text='Tools', )
        # Adding File Menu and commands
        # , font = ('helvetica',11))
        file2 = tk.Menu(self.tool_menubar, tearoff=0, bg='#fcfdfd')
        self.tool_menubar["menu"] = file2
        self.tool_menubar.place(
            relx=0.15, rely=0, relwidth=0.1, relheight=0.03)
        # forming and placing canvas and little canvas
        self.behindcanvas = tk.Canvas(
            self.canvas, bd=0, highlightthickness=0, bg='#CC5F00')
        self.behindcanvas.place(relx=0.6, rely=0.038,
                                relwidth=0.39, relheight=0.96)
        #######################
        self.littlecanvas2_label = tk.Canvas(self.canvas, bd=0, bg='#CC5F00',
                                             selectborderwidth=0,  highlightthickness=0, insertwidth=0)
        self.littlecanvas2_label.place(
            relx=0.003, rely=0.011, relwidth=0.18, relheight=0.027)
        self.littlecanvas2_label_id = self.littlecanvas2_label.create_text(
            10, 5, anchor="nw", fill='white')
        ...
        self.behindcanvas2 = tk.Canvas(
            self.canvas, bd=0, highlightthickness=0, bg='#CC5F00')
        self.behindcanvas2.place(
            relx=0.003, rely=0.038, relwidth=0.37, relheight=0.96)
        ##########################################
        self.littlecanvas_label = tk.Canvas(self.canvas, bd=0, bg='#CC5F00',
                                            selectborderwidth=0,  highlightthickness=0, insertwidth=0)
        self.littlecanvas_label.place(
            relx=0.6, rely=0.011, relwidth=0.18, relheight=0.027)
        self.littlecanvas_label_id = self.littlecanvas_label.create_text(
            10, 5, anchor="nw", fill='white')
        ...
        self.littlecanvas_a_label = tk.Canvas(self.canvas, bd=0, bg='#CC5F00',
                                              selectborderwidth=0,  highlightthickness=0, insertwidth=0)
        self.littlecanvas_a_label.place(
            relx=0.375, rely=0.518, relwidth=0.08, relheight=0.027)
        self.littlecanvas_a_label_id = self.littlecanvas_a_label.create_text(
            10, 5, anchor="nw", fill='white')
        self.littlecanvas_a_label.itemconfig(
            self.littlecanvas_a_label_id, text='Context list', font='helvetica 12 bold')
        ############################################
        self.behindcanvas_a = tk.Canvas(
            self.canvas, bd=0, highlightthickness=0, bg='#CC5F00')
        self.behindcanvas_a.place(
            relx=0.375, rely=0.038, relwidth=0.223, relheight=0.45)
        ######################################
        self.littlecanvas_a = tk.Canvas(self.behindcanvas_a, bd=0, bg='white',
                                        selectborderwidth=0, highlightthickness=0, insertwidth=0)
        self.littlecanvas_a.place(
            relx=0.005, rely=0.005, relwidth=0.99, relheight=0.99)
        #####################################################
        self.behindcanvas_3 = tk.Canvas(
            self.canvas, bd=0, highlightthickness=0, bg='#CC5F00')
        self.behindcanvas_3.place(
            relx=0.375, rely=0.545, relwidth=0.223, relheight=0.45)
        # 3
        ...
        self.littlecanvas3 = tk.Canvas(self.behindcanvas_3, bd=0, bg='white',
                                       selectborderwidth=0, highlightthickness=0, insertwidth=0)
        self.littlecanvas3.place(
            relx=0.005, rely=0.005, relwidth=0.99, relheight=0.901)
        self.littlecanvas3_id = self.littlecanvas3.create_text(
            10, 10, anchor="nw", fill='#0A3200')
        self.littlecanvas3.itemconfig(
            self.littlecanvas3_id, text="No contexts chosen for results. \n\nTo add a context to the results list right click on \nthe context you want then select 'add to list'",  font='helvetica 12 bold')

        ###############################################
        self.littlecanvas3_label = tk.Canvas(self.canvas, bd=0, bg='#CC5F00',
                                             selectborderwidth=0,  highlightthickness=0, insertwidth=0)
        self.littlecanvas3_label.place(
            relx=0.375, rely=0.011, relwidth=0.14, relheight=0.027)
        self.littlecanvas3_label_id = self.littlecanvas3_label.create_text(
            10, 5, anchor="nw", fill='white')
        ...
        self.button2.place(relx=0.008, rely=0.92,
                           relwidth=0.402, relheight=0.07)
        self.button2a = ttk.Button(self.behindcanvas_3, text="HPD intervals",
                                   command=self.get_hpd_interval)
        self.button2a.place(relx=0.415, rely=0.92,
                            relwidth=0.322, relheight=0.07)
        self.button3 = ttk.Button(self.behindcanvas_3, text="Clear list",
                                  command=self.clear_results_list)
        self.button3.place(relx=0.74, rely=0.92,
                           relwidth=0.252, relheight=0.07)
        self.ResultList = [
            "Add to results list",
            "Get time elapsed",
        ]
    ...
    def node_finder(self, currentevent):
        ...
        #  plot1.ylabel('Probability density')
                ...
                plot1.title.set_text('Time elapsed between ' + str(
                    self.phase_len_nodes[0]) + ' and ' + str(self.phase_len_nodes[1]))
        ...
        #       self.fig.set_tight_layout(True)
                ...
                self.tree_phases = ttk.Treeview(
                    self.littlecanvas_a, columns=columns, show='headings')
                # define headings
                ...
                for i in refs:
                    hpd_str = hpd_str + \
                        str(np.abs(interval[i-1])) + " - " + \
                        str(np.abs(interval[i])) + " Cal BP "
                # add data to the treeview
                intervals.append(
                    (self.phase_len_nodes[0], self.phase_len_nodes[1], hpd_str))
                for contact in intervals:
                    self.tree_phases.insert('', tk.END, values=contact)
                self.tree_phases.place(relx=0, rely=0, relwidth=1)
                # add a scrollbar
                scrollbar = ttk.Scrollbar(
                    self.littlecanvas_a, orient=tk.VERTICAL, command=self.tree_phases.yview)
                self.tree_phases.configure(yscroll=scrollbar.set)
                scrollbar.grid(row=0, column=1, sticky='nsew')
            self.ResultList.remove("Get time elapsed between " +
                                   str(self.phase_len_nodes[0]) + ' and another context')
            self.testmenu2 = ttk.OptionMenu(
                self.littlecanvas2, self.variable, self.ResultList[0], *self.ResultList, command=self.node_finder)
            self.phase_len_nodes = []
        ...
        if self.variable.get() == "Get time elapsed":
            if len(self.phase_len_nodes) == 1:
                ...
                self.phase_len_nodes = []
            self.phase_len_nodes = np.append(self.phase_len_nodes, x)
            self.ResultList.append("Get time elapsed between " +
                                   str(self.phase_len_nodes[0]) + ' and another context')
            self.testmenu2 = ttk.OptionMenu(
                self.littlecanvas2, self.variable, self.ResultList[0], *self.ResultList, command=self.node_finder)
        self.littlecanvas3.delete(self.results_text)
    ...
    def onRight(self, *args):
        ...
        # Here we fetch our X and Y coordinates of the cursor RELATIVE to the window
        self.cursorx2 = int(
            self.littlecanvas2.winfo_pointerx() - self.littlecanvas2.winfo_rootx())
        self.cursory2 = int(
            self.littlecanvas2.winfo_pointery() - self.littlecanvas2.winfo_rooty())
    ...
    def get_hpd_interval(self):
        ...
        if len(self.results_list) != 0:
            ...
            if mcmc_check == 'mcmc_loaded':
                ...
                self.tree_phases.heading(
                    'hpd_interval', text=str(USER_INP) + '% HPD interval')
                intervals = []
                for i, j in enumerate(list(set(self.results_list))):
                    node = str(j)
                    interval = list(mcmc.HPD_interval(
                        np.array(startpage.resultsdict[j][1000:]), lim=self.lim))
                # define headings
                    hpd_str = ""
                    refs = [k for k in range(len(interval)) if k % 2]
                    for i in refs:
                ...
                self.tree_phases.configure(yscroll=scrollbar.set)
                scrollbar.grid(row=0, column=1, sticky='nsew')
                self.littlecanvas_a.create_text(
                    150, 80, text=hpd_str, fill='#0A3200')
    ...
    def chronograph_render_post(self):
        ...
        #   self.imscale2 = 1.0  # scale for the canvaas image
            ...
            self.imscale2 = min(
                921/self.image2.size[0], 702/self.image2.size[1])
            # rescale all canvas objects
            self.littlecanvas2.scale('all', 0, 0, self.delta2, self.delta2)
            self.show_image2()
...
#        self.metatext = ""
       ...
       # self.imscale2 = 1.0  # scale for the canvaas image
        ...
        self.graphcanvas = tk.Canvas(self.canvas, bd=0, bg='white',
                                     selectborderwidth=0, highlightthickness=0, insertwidth=0)
        self.graphcanvas.place(relx=0.02, rely=0.05,
                               relwidth=0.35, relheight=0.9)
        ...
        self.residcanvas = tk.Canvas(self.canvas, bd=0, bg='white',
                                     selectborderwidth=0, highlightthickness=0, insertwidth=0)
        self.residcanvas.place(relx=0.4, rely=0.42,
                               relwidth=0.35, relheight=0.08)
        self.intrucanvas = tk.Canvas(self.canvas, bd=0, bg='white',
                                     selectborderwidth=0, highlightthickness=0, insertwidth=0)
        self.intrucanvas.place(relx=0.4, rely=0.54,
                               relwidth=0.35, relheight=0.08)
        ...
        if startpage.graph != None:
            ...
            # rescale all canvas objects
            self.graphcanvas.scale('all', 0, 0, self.delta2, self.delta2)
            self.show_image2()
        self.graphcanvas.update()
        ...
        # only with Linux, wheel scroll down
        self.graphcanvas.bind('<Button-4>', self.wheel2)
    ...
    def popup4_wrapper(self, controller):
        ...
        self.popup4 = popupWindow4(self, controller, self.resid_list,
                                   self.intru_list, self.node_del_tracker, self.graphcopy)
        self.top.destroy()
    ...
    def load_graph(self):
        ...
        if phase_true == 1:
            ...
            self.image = imgrender(
                self.graphcopy, self.graphcanvas.winfo_width(), self.graphcanvas.winfo_height())
    ...
    def wheel2(self, event):
        ...
        if event.num == 5 or event.delta == -120:  # scroll down
            ...
            scale2 /= self.delta2
        if event.num == 4 or event.delta == 120:  # scroll up
            i = min(self.graphcanvas.winfo_width(),
                    self.graphcanvas.winfo_height())
            if i < self.imscale2:
                return  # 1 pixel is bigger than the visible area
            self.imscale2 *= self.delta2
            scale2 *= self.delta2
    #    print(scale2)
        # rescale all canvas objects
        self.graphcanvas.scale('all', 0, 0, scale2, scale2)
        self.show_image2()
    ...
    def show_image2(self):
        ...
        startpage = self.controller.get_page('StartPage')
        startpage.update_idletasks()
        bbox1 = [0, 0, int(self.image.size[0]*self.imscale2),
                 int(self.image.size[1]*self.imscale2)]
        # Remove 1 pixel shift at the sides of the bbox1
        bbox1 = (bbox1[0] + 1, bbox1[1] + 1, bbox1[2] - 1, bbox1[3] - 1)
    ...
    def resid_node_click(self, event):
        ...
        startpage.update_idletasks()
        self.cursorx2 = int(
            self.graphcanvas.winfo_pointerx() - self.graphcanvas.winfo_rootx())
        self.cursory2 = int(
            self.graphcanvas.winfo_pointery() - self.graphcanvas.winfo_rooty())
        x_scal = self.cursorx2 + self.transx2
        y_scal = self.cursory2 + self.transy2
        node = self.nodecheck(x_scal, y_scal)
        outline = nx.get_node_attributes(self.graphcopy, 'color')
        ...
        if ((node in self.resid_list) == True) and (self.modevariable != 'intru'):
            ...
            self.intru_list.append(node)
            outline[node] = 'green'
        self.resid_label = ttk.Label(self.residcanvas, text=str(
            self.resid_list).replace("'", "")[1:-1])
        self.resid_label.place(relx=0, rely=0, relwidth=1, relheight=1)
        self.intru_label = ttk.Label(self.intrucanvas, text=str(
            self.intru_list).replace("'", "")[1:-1])
        self.intru_label.place(relx=0, rely=0, relwidth=1, relheight=1)
        ...
        if phase_true == 1:
            ...
            imgrender(self.graphcopy, self.graphcanvas.winfo_width(),
                      self.graphcanvas.winfo_height())
