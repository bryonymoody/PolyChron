...
def strat_rel(site_dict, key, i_index, THETAS, CONTEXT_NO):
    ...
    if len(a_strat) > 0:
        stratup = max([THETAS[i] for i in a_strat])
    else:
        stratup = site_dict[key]["boundaries"][0]
    if len(b_strat) > 0:
        stratlow = min([THETAS[i] for i in b_strat])
    else:
...
#    phase_len = site_dict[key]["boundaries"][1] - site_dict[key]["boundaries"][0] #phase length
   ...
   # t1 = time.time()
    if (date_ref >= len(like1) or phase_len == 0): #checking we haven't sampled outside the realms of the likelihood
        x_temp = 0
        x_len = 0
    else:
        x_temp = like2[date_ref]
        if (min(strat_up, alpha) == alpha) and (max(strat_low, beta) == beta):
            x_len = (x_temp/phase_len)
        elif np.all(temp_vec_2 == 0):
            x_temp = 0
            x_len = 0
        else:
            x_len = (x_temp/phase_len)*(temp_vec.sum()/temp_vec_2.sum())
  #  if (min(strat_up, alpha) == alpha) and (max(strat_low, beta) == beta):
   #     print(x_len == (x_temp/phase_len))
    if math.isnan(x_len):
        x_temp = 0
        x_len = 0
 #   print(str(key) + ": " + str(temp_vec.sum()) + ", " + str(temp_vec_2.sum()) + "," + str((min(strat_up, alpha) == alpha)))
  #  print(min(strat_up, alpha) == alpha)
  #  print(max(strat_low, beta) == beta)
...
def post_h(A, P, site_dict, THETAS, CONTEXT_NO, RCD_ERR, RCD_EST, CALIBRATION):
  ...
  #  print(['%.9f' % elem for elem in hh_1[0:7]])
 #   print(['%.9f' % elem for elem in hh_2[0:7]])
    return hh_1, hh_2

def post_h_1_2(A, P, site_dict, THETAS, CONTEXT_NO, RCD_ERR, RCD_EST, CALIBRATION):
        """calculates acceptance probability"""
        h_vec = [dict_seek_ordered(A, P, j[0], i, site_dict, THETAS, CONTEXT_NO, RCD_ERR, RCD_EST, CALIBRATION)
                 for i in site_dict.keys()
                 for j in enumerate(site_dict[i]["dates"])]
...
def post_h_theta_phi_4(i, j, site_dict):
...
def dict_update(test_dict_2, post_t, post_p, inter, inter2, POST_PHASE, PHI_REF, TOPO_SORT):
  ...
  #  count = 0
    ...
    for i_2 in PHI_REF:
        ...
        for j_up in enumerate(test_dict_2[i_2]["dates"]):
            x = np.where(TOPO_SORT == j_up[1][2])[0][0]
            j_up[1][0] = post_t[x][inter]
    return test_dict_2
...
def phase_bd_init_func(key_reff, phi_reff, theta_initt, prev_phases, A, P):
    ...
    for j in range(1, len(phi_reff)):
        ...
        if prev_phases[j] != "abutting":
            ...
            a_temp = [a_temp]
        [PHASE_INITS.append(ref_ind) for ref_ind in a_temp]
        if prev_phases[j] == "overlap":
            print(j)
        #    print("hey")
            print(len(PHASE_INITS))
            ref_len = len(PHASE_INITS) -1
            temp_lims = PHASE_INITS[ref_len-1]
            PHASE_INITS[ref_len-1] = PHASE_INITS[ref_len]
            PHASE_INITS[ref_len] = temp_lims
...
def upp_samp_3(theta_samp, phis_samp, m, A, P, SAMP_VEC_TRACK, KEY_REF, PHI_REF, CONT_TYPE):
    ...
    limits = [max([theta_samp[i] for i, j in enumerate(KEY_REF) if
              ...
              min([theta_samp[i] for i, j in enumerate(KEY_REF)
                   if j == PHI_REF[SAMP_VEC_TRACK[m-1]] and CONT_TYPE[i] == 'normal'] + [phis_samp[m-1]])]
    _ = phis_samp
    return limits
...
def upp_samp_8(theta_samp, phis_samp, m, A, P, SAMP_VEC_TRACK, KEY_REF, PHI_REF, CONT_TYPE):
    """ upper sample 8 - phase with previous 2 overlapping, same as overlap samp_5??, should delete"""
    in_phase_dates = [theta_samp[i] for i, j in enumerate(KEY_REF) if
                      j == PHI_REF[SAMP_VEC_TRACK[m]] and CONT_TYPE[i] == 'normal']
    in_phase_dates.append(phis_samp[m-1])
    limits = [max(in_phase_dates), phis_samp[m-2]]
...
def low_samp_5(theta_samp, phis_samp, m, A, P, SAMP_VEC_TRACK, KEY_REF, PHI_REF, CONT_TYPE):
    """lower sample 5 next phase gap and prev phase overlap"""
    in_phase_dates = [theta_samp[i] for i, j in enumerate(KEY_REF) if
                      j == PHI_REF[SAMP_VEC_TRACK[m]] and CONT_TYPE[i] == 'normal']
    in_phase_dates.append(phis_samp[m-2])
    in_phase_dates.append(phis_samp[m-1])
...
def low_samp_6(theta_samp, phis_samp, m, A, P, SAMP_VEC_TRACK, KEY_REF, PHI_REF, CONT_TYPE):
    """lower sample 6 very oldest beta and prev phase anything but overlap"""
...
def low_samp_7(theta_samp, phis_samp, m, A, P, SAMP_VEC_TRACK, KEY_REF, PHI_REF, CONT_TYPE):
    """lower sample 7 very oldest beta and prev phase overlap"""
    in_phase_dates = [theta_samp[i] for i, j in enumerate(KEY_REF) if
                      j == PHI_REF[SAMP_VEC_TRACK[m]] and CONT_TYPE[i] == 'normal']
    in_phase_dates.append(phis_samp[m-2])
    in_phase_dates.append(phis_samp[m-1])
...
def gibbs_code(iter_num, RESULT_VEC, A, P, KEY_REF, PHI_REF, STRAT_VEC, CONTEXT_NO, TOPO_SORT, PREV_PHASE, POST_PHASE, PHI_SAMP_DICT, CONT_TYPE):
    ...
    for i in range(0, iter_num):
        for k in range(len(THETA_INITS)):
            SITE_DICT_TEST_1 = dict_update(SITE_DICT_TEST_1, POST_THETAS, POST_PHIS, i, i, POST_PHASE, PHI_REF, TOPO_SORT)
            key = KEY_REF[k]
...
def dict_test_func(dict_for_test, step, node = None, new_date = None):
    check = []
    for key in ["M2", "M3"]:
        bounds = dict_for_test[key]["boundaries"]
        dates = [(i[0], i[2]) for i in dict_for_test[key]["dates"]]
      #  dates_label = [i[2] for i in dict_for_test[key]["dates"]]
        check = [(i[0] > bounds[0]) and (i[0] < bounds[1]) for i in dates]
        if np.prod(check)==0:
            print(check)
            print(dates)
            print(new_date)
...
#%%V
def step_1_squeeze(A, P, i, PREV_PROB_TEST, K, ACCEPT, SITE_DICT_TEST_1, SITE_DICT_TEST_2, THETAS, POST_THETAS, POST_PHIS, KEY_REF, CONTEXT_NO, POST_PHASE, PHI_REF, RCD_ERR, RCD_EST, CALIBRATION, ALL_SAMPS_CONT, ALL_SAMPS_PHI, PHIS_VEC, CONT_TYPE, TOPO_SORT):
  #  print("step1")
...
#    print(CONTEXT_NO[k])
   ...
   # print(strat_single)
    ...
    SITE_DICT_TEST_2 = dict_update(SITE_DICT_TEST_2, POST_THETAS, POST_PHIS, i+1, i, POST_PHASE, PHI_REF, TOPO_SORT)
    dict_test_func(SITE_DICT_TEST_2, 1, CONTEXT_NO[k], THETAS[k])
...
def step_2_squeeze(i, prob_1_test, M, PHI_SAMP_DICT, POST_S, R, PHIS_VEC, SITE_DICT_TEST_3, THETAS, POST_THETAS, POST_PHIS, STEP_1, STEP_2, STEP_3, A, P, SAMP_VEC_TRACK, PHI_ACCEPT, KEY_REF, PHI_REF, CONTEXT_NO, RCD_ERR, POST_PHASE, RCD_EST, CALIBRATION, ALL_SAMPS_CONT, ALL_SAMPS_PHI, CONT_TYPE, TOPO_SORT):
  #  print("step2")
 ...
 #   print([int(i) for i in PHIS_VEC])
    ...
    f_phi2 = (s2**(2-M))/(R-s2)
    SITE_DICT_TEST_3 = dict_update(SITE_DICT_TEST_3, POST_THETAS, POST_PHIS, i+1, i+1, POST_PHASE, PHI_REF, TOPO_SORT)
    dict_test_func(SITE_DICT_TEST_3, 2)
...
def step_3_squeeze(A, P, i, backup_b_test, prob_2_test, S, ACCEPT, POST_S, PHIS_VEC, SITE_DICT_TEST_4, THETAS, POST_THETAS, POST_PHIS, PHI_ACCEPT, POST_PHASE, PHI_REF, CONTEXT_NO, RCD_ERR, RCD_EST, CALIBRATION, ALL_SAMPS_CONT, ALL_SAMPS_PHI, TOPO_SORT):
 ...
 #   print("step3") 
    ...
    SITE_DICT_TEST_4 = dict_update(SITE_DICT_TEST_4, POST_THETAS, POST_PHIS, i+2, i+2, POST_PHASE, PHI_REF, TOPO_SORT)
    dict_test_func(SITE_DICT_TEST_4, 3)
...
def step_4_squeeze(A, P, i, b_test, prob_3_test, ACCEPT, POST_S, R, PHIS_VEC, SITE_DICT_TEST_5, THETAS, POST_THETAS, POST_PHIS, PHI_ACCEPT, POST_PHASE, PHI_REF, CONTEXT_NO, RCD_ERR, RCD_EST, CALIBRATION, ALL_SAMPS_CONT, ALL_SAMPS_PHI, TOPO_SORT):
   # print("step4")
    ...
    SITE_DICT_TEST_5 = dict_update(SITE_DICT_TEST_5, POST_THETAS, POST_PHIS, i+3, i+3, POST_PHASE, PHI_REF, TOPO_SORT)
    dict_test_func(SITE_DICT_TEST_5, 4)
...
def squeeze_model(PHI_SAMP_DICT, RESULT_VEC, A, P, RCD_ERR, KEY_REF, STRAT_VEC, CONTEXT_NO, TOPO_SORT, PREV_PHASE, POST_PHASE, PHI_REF, RCD_EST, CALIBRATION, CONT_TYPE):
    ...
    while acept_prob > 0:
        ...
        STEP_1, STEP_2, STEP_3, SAMP_VEC_TRACK = how_to_phase_samp(POST_PHASE, PREV_PHASE)
        print(STEP_1, STEP_2, STEP_3, SAMP_VEC_TRACK)
        ########################### figures out how to sample based on phase relationships
        K = len(THETAS)
        M = len(PHIS_VEC)
        S = min(RCD_ERR)
       ...
       #     print((max([len(i) for i in ACCEPT])))          
            ...
            print("Acceptance probability min: " + str(min(CHECK_ACCEPT)*100))
            print("Acceptance probability max: " + str(max(CHECK_ACCEPT)*100))
